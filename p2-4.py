...
def findAccuracy(layers,x,y, out_dir,act_fn):
    ...
    for i in range(1,len(layers)):
        ...
        if act_fn == "sigmoid" or i == len(layers)-1 :
            out = 1/(1+np.exp(-out))
        else:
            out[out<=0]=0
            # out[:][:] = max(0,out[:][:])
...
def neuralNet(trainx,trainy,testx,testy,out_dir,batch_size,no_features,architecture,no_targets,eta,act_fn):
    x1 = np.load(trainx)
    y1 = np.load(trainy)
    start = time.time()
    ...
    acc1 = 100*findAccuracy(layers,x,y1,"out_train.txt",act_fn)
    ...
    acc2 = 100*findAccuracy(layers,x2,y2,"out_test.txt",act_fn)
    print(acc1,acc2)
...
for s in sizes:
    acc1,acc2,dur = neuralNet(sys.argv[1],\
        ...
        sys.argv[4],\
        sys.argv[5],\
        100,784,[s,s,10],10,0.01,"relu")
    acc_train1.append(acc1)
    acc_test1.append(acc2)
    times1.append(dur)
...
for s in sizes:
    acc1,acc2,dur = neuralNet(sys.argv[1],\
        ...
        sys.argv[4],\
        sys.argv[5],\
        100,784,[s,s,10],10,0.5,"sigmoid")
    acc_train2.append(acc1)
    acc_test2.append(acc2)
    times2.append(dur)

import matplotlib.pyplot as plt
plt.figure(1)
plt.subplot(211)
plt.plot(sizes,acc_train1,"-b",label = "ACC-Train-Relu")
plt.plot(sizes,acc_train2,"-g",label = "ACC-Train-Sig")
plt.legend(loc="lower right")
plt.subplot(212)
plt.plot(sizes,acc_test1,"-b",label = "ACC-Test-Relu")
plt.plot(sizes,acc_test1,"-g",label = "ACC-Test-Sig")
plt.legend(loc="lower right")
plt.savefig('acc.png')

plt.figure(2)
plt.plot(sizes,times1,"-b",label = "ACC-Time-Relu")
plt.plot(sizes,times2,"-g",label = "ACC-Time-Sig")
plt.legend(loc="lower right")
...
# sigmoid 99.08333333333333 91.42